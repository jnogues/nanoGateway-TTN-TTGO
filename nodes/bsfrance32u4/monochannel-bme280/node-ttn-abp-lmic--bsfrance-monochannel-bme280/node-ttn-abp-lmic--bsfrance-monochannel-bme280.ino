...
//OJO, descativar beacon i pings a src/lmic/config.h per estalviar memoria #define DISABLE_PING #define DISABLE_BEACONS
...
// network.
static const PROGMEM u1_t NWKSKEY[16] = { 0x00, 0x00, 0x00, 0x71, 0x96, 0x7D, 0xE8, 0x40, 0x81, 0xD7, 0x1D, 0xFF, 0xC7, 0xF4, 0x47, 0x6A };

// LoRaWAN AppSKey, application session key
// This is the default Semtech key, which is used by the early prototype TTN
// network.
static const u1_t PROGMEM APPSKEY[16] = { 0x00, 0x00, 0xC0, 0xA6, 0xE3, 0x42, 0x34, 0xBB, 0x58, 0x1C, 0x80, 0x5E, 0x18, 0xAE, 0x4F, 0x84 };

// LoRaWAN end-device address (DevAddr)
static const u4_t DEVADDR = 0x00011C0E ; // <-- Change this address for every node!
...
void setup() {
    ...
    timer.setInterval(5000L, TemperatureBME);
    ...
    LMIC_setClockError(MAX_CLOCK_ERROR * 10 / 100); // 1 a 10
...
void TemperatureBME()
{
  ...
  float p=bme280.getPressure()/100.0F;
  P=int(p);
  Serial.print("Pressió (hPa): ");
  Serial.println(p);
...
void do_send(osjob_t* j) {
  ...
  // Prepare buffer 
    byte dades[5];
    memset(dades,0,5);//esborra estat
    dades[0]=T;
    dades[1]=H;
    dades[2]=highByte(P);
    dades[3]=lowByte(P);
    ...
    //dataToSendString = String(T);
    //dataToSendString.toCharArray(dades,5);

    // Prepare upstream data transmission at the next possible time.
    // Parameters are port, data, length, confirmed
    LMIC_setTxData2(1, dades, 5, 0);
